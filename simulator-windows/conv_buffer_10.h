#ifndef _CONV_BUFFER_10
#define _CONV_BUFFER_10
...
// convolution buffer 10, input channels 80, output channels 80
// image size 8

SC_MODULE(conv_buffer_10) {
	...
	float buffer[CHANNELS_80][IMAGE_SIZE_8*KERNEL_SIZE];
					...
					for (int j = 1; j < KERNEL_SIZE; j++) {
						for (int k = 0; k < KERNEL_SIZE; k++) {
							...
							if ((y - 1 + k < 0) || (y - 1 + k >= IMAGE_SIZE_8))
								continue;
							tmp_data[j][k] = buffer[i][((x - 1 + j) % KERNEL_SIZE)*IMAGE_SIZE_8 + (y - 1 + k)];
						}
					}
				}
