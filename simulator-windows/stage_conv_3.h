...
#ifndef _STAGE_CONV_3
#define _STAGE_CONV_3
...
// fifth convolution layer, input channel CHANNELS_32, output channel CHANNELS_32, pooling POOLING_SIZE_1

// #define POOLING_SIZE POOLING_SIZE_1

SC_MODULE(stage_conv_3) {
	...
	CROSSBAR cb;
	float pooling_buffer[CHANNELS_32][IMAGE_SIZE_32*POOLING_SIZE_1];
	int pooling_pointer;
	int print;

	// read crossbar data from file
	void init_crossbar() {
		// read from convolution layer 3
		float* cell = new float[CROSSBAR_L*CROSSBAR_W];
		string filename = "./weights/weight_3.csv";
		...
		delete[] cell;
		cout << "load weights 3 complete. " << filename << endl;
		// cb.printcrossbar();
	...
	void max_pooling() {
			...
			for (int i = 0; i < CHANNELS_32; i++) {
				...
				for (int j = 0; j < POOLING_SIZE_1; j++){
					for (int k = 0; k < POOLING_SIZE_1; k++){
						float element = pooling_buffer[i][(x - j) % POOLING_SIZE_1*IMAGE_SIZE_32 + (y - k)];
						if (element > _max)
							_max = element;
					}
				}
		...
		pooling_pointer++;
		if (pooling_pointer >= IMAGE_SIZE_32 * POOLING_SIZE_1)
			pooling_pointer = 0;
	}
	...
	void stage_conv_run() {
		// with ad & da
		// read data
		float input_buff[INPUT_SIZE*CHANNELS_32] = { 0.0 };
		float _max = 0.0;
        ...
        for (int i = 0; i < AD_WIDTH/DA_WIDTH; ++i){
            ...
            for (int j = 0; j < INPUT_SIZE*CHANNELS_32; ++j){
                ...
                tmp_input[CROSSBAR_L - INPUT_SIZE * CHANNELS_32 + j] = float(bitnum);
                input_buff[j] = input_buff[j] / pow(2, DA_WIDTH);
            }
            cb.run(tmp_input, tmp_output, false);
            // ad and shift add
            for (int j = 0; j < CROSSBAR_W; ++j){
                float tmp = tmp_output[j] / XB3_I;
                if (tmp > 1)
                	adc.trans(1.0);
                else if (tmp < -1) 
                	adc.trans(-1.0);
...
//			for (j = 0; j < INPUT_SIZE*CHANNELS_32; ++j){
//				int bitnum = static_cast<int>(int(input_buff[j]) & move);
//				dac.trans(bitnum, DA_WIDTH);
//				tmp_input[CROSSBAR_L * i + CROSSBAR_L - INPUT_SIZE * CHANNELS_32 + j] = float(bitnum);
//				input_buff[j] = input_buff[j] / pow(2, DA_WIDTH);
//			}
//		}
