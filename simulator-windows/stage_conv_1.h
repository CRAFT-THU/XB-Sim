...
// fifth convolution layer, input channel CHANNELS_3, output channel CHANNELS_32, pooling POOLING_SIZE_1
...
SC_MODULE(stage_conv_1) {
	...
	void init_crossbar() {
		...
		string filename = "./weights/weight_1.csv";
		...
		delete[] cell;
		cout << "load weights 1 complete. " << filename << endl;
		// cb.printcrossbar();
	...
	void stage_conv_run() {
		// with ad & da
		// read data
		float input_buff[INPUT_SIZE*CHANNELS_3] = { 0.0 };
		float _max = 0.0;
        ...
        for (int i = 0; i < AD_WIDTH/DA_WIDTH; ++i){
            ...
            for (int j = 0; j < INPUT_SIZE*CHANNELS_3; ++j){
                ...
                tmp_input[CROSSBAR_L - INPUT_SIZE * CHANNELS_3 + j] = float(bitnum);
...
//			for (j = 0; j < INPUT_SIZE*CHANNELS_3; ++j){
//				int bitnum = static_cast<int>(int(input_buff[j]) & move);
//				dac.trans(bitnum, DA_WIDTH);
//				tmp_input[CROSSBAR_L * i + CROSSBAR_L - INPUT_SIZE * CHANNELS_3 + j] = float(bitnum);
//				input_buff[j] = input_buff[j] / pow(2, DA_WIDTH);
//			}
//		}
