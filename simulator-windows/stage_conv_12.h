...
#ifndef _STAGE_CONV_12
#define _STAGE_CONV_12
...
SC_MODULE(stage_conv_12) {
	...
	void init_crossbar() {
		// read from convolution layer 12
		float* cell = new float[CROSSBAR_L*CROSSBAR_W];
		string filename = "./weights/weight_12.csv";
		...
		delete[] cell;
		cout << "load weights 12 complete. " << filename << endl;
		// cb.printcrossbar();
	...
	void stage_conv_run() {
		...
		for (int i = 0; i < AD_WIDTH/DA_WIDTH; ++i){
			...
			// ad and shift add
			for (int j = 0; j < CROSSBAR_W; ++j){
				float tmp = tmp_output[j] / XB12_I;
				if (tmp > 1)
					adc.trans(1.0);
				else {
					adc.trans(tmp);
				}
				// float tmp = adc.AD_out / (XB12_I * 0.15); // divide by xb_i?
