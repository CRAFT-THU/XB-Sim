...
#ifndef _STAGE_CONV_6
#define _STAGE_CONV_6
...
// fifth convolution layer, input channel CHANNELS_48, output channel CHANNELS_80, pooling POOLING_SIZE_1

// #define POOLING_SIZE POOLING_SIZE_1

SC_MODULE(stage_conv_6) {
	...
	CROSSBAR cb;
	float pooling_buffer[CHANNELS_80][IMAGE_SIZE_16*POOLING_SIZE_1];
	int pooling_pointer;
	int print;

	// read crossbar data from file
	void init_crossbar() {
		// read from convolution layer 6
		float* cell = new float[CROSSBAR_L*CROSSBAR_W];
		string filename = "./weights/weight_6.csv";
		...
		delete[] cell;
		cout << "load weights 6 complete. " << filename << endl;
		// cb.printcrossbar();
	...
	void max_pooling() {
			...
			for (int i = 0; i < CHANNELS_80; i++) {
				...
				for (int j = 0; j < POOLING_SIZE_1; j++){
					for (int k = 0; k < POOLING_SIZE_1; k++){
						float element = pooling_buffer[i][(x - j) % POOLING_SIZE_1*IMAGE_SIZE_16 + (y - k)];
						if (element > _max)
							_max = element;
					}
				}
		...
		pooling_pointer++;
		if (pooling_pointer >= IMAGE_SIZE_16 * POOLING_SIZE_1)
			pooling_pointer = 0;
	}
...
//			for (j = 0; j < INPUT_SIZE*CHANNELS_48; ++j){
//				int bitnum = static_cast<int>(int(input_buff[j]) & move);
//				dac.trans(bitnum, DA_WIDTH);
//				tmp_input[CROSSBAR_L * i + CROSSBAR_L - INPUT_SIZE * CHANNELS_48 + j] = float(bitnum);
//				input_buff[j] = input_buff[j] / pow(2, DA_WIDTH);
//			}
//		}
