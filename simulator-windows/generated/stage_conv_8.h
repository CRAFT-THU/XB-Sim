...
#ifndef _STAGE_CONV_8
#define _STAGE_CONV_8
...
// fifth convolution layer, input channel CHANNELS_80, output channel CHANNELS_80, pooling POOLING_SIZE_1

// #define POOLING_SIZE POOLING_SIZE_1

SC_MODULE(stage_conv_8) {
	...
	void init_crossbar() {
		// read from convolution layer 8
		float* cell = new float[CROSSBAR_L*CROSSBAR_W];
		string filename = "./weights/weight_8.csv";
		...
		delete[] cell;
		cout << "load weights 8 complete. " << filename << endl;
		// cb.printcrossbar();
...
//			for (j = 0; j < INPUT_SIZE*CHANNELS_80; ++j){
//				int bitnum = static_cast<int>(int(input_buff[j]) & move);
//				dac.trans(bitnum, DA_WIDTH);
//				tmp_input[CROSSBAR_L * i + CROSSBAR_L - INPUT_SIZE * CHANNELS_80 + j] = float(bitnum);
//				input_buff[j] = input_buff[j] / pow(2, DA_WIDTH);
//			}
//		}
